...
-(void)viewWillAppear:(BOOL)animated
{
    ...
    if (mDefaultText && mDefaultText.length > 0) {
        messageTextField.text = mDefaultText;        
...
- (void)textChanged:(NSNotification *)notification{
    int maxLength = kWeiboMaxLength-mTailText.length;
...
- (void)postNewStatus
{
    ...
    draft.text = [NSString stringWithFormat:@"%@%@",messageTextField.text,mTailText];
